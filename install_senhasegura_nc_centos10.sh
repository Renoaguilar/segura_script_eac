#!/usr/bin/env bash
# ==========================================================
# Senhasegura Network Connector - Installer + Doctor (CentOS Stream 10 / RHEL 10)
# - Instala Docker CE + compose plugin; si no hay paquetes, usa Podman (podman-docker)
# - Genera docker-compose.yml SIN comillas en environment y SIN 'ports:'
# - Levanta el agente y ejecuta autotest (contenedor, ENV, reach 51445, firewalld)
# ==========================================================
set -euo pipefail
...
SNC_FINGERPRINT="${SNC_FINGERPRINT//[[:space:]]/}"; PAM_IPS="${PAM_IPS// /}"; IS_SECONDARY="${IS_SECONDARY,,}"
...
# ---------- Instalar runtime: Docker CE -> fallback Podman ----------
HAVE_COMPOSE="no"; RUNTIME="none"
say "$INFO" "Instalando Docker CE (si hay paquetes para este release)"
...
if dnf list --available docker-ce >/dev/null 2>&1; then
  ...
  say "$WARN" "Paquetes docker-ce no disponibles; usando Podman + compatibilidad"
...
if ! has docker; then
  echo -e "$FAIL No se obtuvo CLI 'docker' (ni podman-docker). Revisa repositorios/red."
  exit 1
fi
# Si aún no detectamos compose, reintenta detección
...
# ---------- Generar docker-compose.yml (SIN comillas, SIN ports) ----------
...
# localizar contenedor por etiqueta de compose; si no, por nombre
CID="$(docker ps -q -f "label=com.docker.compose.service=${SERVICE_NAME}" | head -n1)"
[[ -z "$CID" ]] && CID="$(docker ps -q --filter "name=${SERVICE_NAME}" | head -n1)"
if [[ -z "$CID" ]]; then
...
for ip in "${PAMS[@]}"; do
  ...
  if nc -zw3 "$ip" 51445; then
    ...
    say "$WARN" "Sin salida a ${ip}:51445 (posible firewall perimetral/proxy)"
    ALL_OK=0
  fi
done

# informar firewalld (no tocar reglas)
